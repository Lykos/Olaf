...
const int c_castle_offset = c_no_colors * Position::c_index_size * PieceSet::c_no_pieces;
const int c_ep_offset = c_castle_offset + 4;
const int c_turn_offset = c_ep_offset + 8;

static uint_fast8_t opening_piece_index(Color color, Piece::piece_index_t piece_index)
{
  uint_fast8_t index = 0;
  switch (piece_index) {
    case PieceSet::c_pawn_index:
      index = 0;
      break;
    case PieceSet::c_knight_index:
      index = 2;
      break;
    case PieceSet::c_bishop_index:
      index = 4;
      break;
    case PieceSet::c_rook_index:
      index = 6;
      break;
    case PieceSet::c_queen_index:
      index = 8;
      break;
    case PieceSet::c_king_index:
      index = 10;
      break;
    default:
      assert(false);
...
uint64_t OpeningBook::opening_zobrist_hash(const ChessBoard& board) const
{
  ...
  for (Color color : c_colors) {
    ...
    for (Piece::piece_index_t piece_index = 0; piece_index < PieceSet::c_no_pieces; ++piece_index) {
      ...
      while (piece_board) {
        const Position position = piece_board.next_position();
        const int hash_index = opening_piece_index(color, piece_index) * Position::c_index_size + position.index();
        hash ^= c_hash_values[hash_index];
      }
    }
  }
  if (board.color_board(Color::White).can_castle_k()) {
    hash ^= c_hash_values[c_castle_offset];
  }
  if (board.color_board(Color::White).can_castle_q()) {
    hash ^= c_hash_values[c_castle_offset + 1];
  }
  if (board.color_board(Color::Black).can_castle_k()) {
    hash ^= c_hash_values[c_castle_offset + 2];
  }
