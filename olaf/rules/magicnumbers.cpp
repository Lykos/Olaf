...
typedef std::array<std::uint64_t, MagicNumbers::c_move_table_size> MoveTable;

static MoveTable c_move_table;

static MagicNumbers::MagicTable create_rook_magic()
{
  return {{
...
static MagicNumbers::MagicTable create_bishop_magic() {
  return {{
...
void MagicNumbers::init()
{
  ...
  assert(size > 0);
  char* const bytes = reinterpret_cast<char*>(&c_move_table);
  file.read(bytes, size);
  file.close();
}

const MagicNumbers::MagicTable MagicNumbers::c_rook_magic = create_rook_magic();

const MagicNumbers::MagicTable MagicNumbers::c_bishop_magic = create_bishop_magic();
