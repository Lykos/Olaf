...
void XBoardWriter::tell_opponent(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellopponent " << message << endl;
}

void XBoardWriter::tell_others(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellothers " << message << endl;
}

void XBoardWriter::tell_all(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellall " << message << endl;
}

void XBoardWriter::tell_user(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "telluser " << message << endl;
}

void XBoardWriter::tell_user_error(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellusererror " << message << endl;
}

void XBoardWriter::askuser(const std::string &rep_tag, const std::string &question)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "askuser " << rep_tag << " " << question << endl;
}

void XBoardWriter::tell_ics(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellics " << message << endl;
}

void XBoardWriter::tell_ics_noalias(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "tellicsnoalias " << message << endl;
}

void XBoardWriter::comment(const std::string &message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "# " << message << endl;
}
...
void XBoardWriter::error(const std::string& type, const std::string& message)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "Error (" << type << "): " << message << endl;
}

void XBoardWriter::illegal_move(const std::string& move)
{
  unique_lock<mutex> lock (m_mutex);
  cout << "Illegal move: " << move << endl;
}

void XBoardWriter::illegal_move(const std::string& reason, const std::string& move)
{
  unique_lock<mutex> lock (m_mutex);
